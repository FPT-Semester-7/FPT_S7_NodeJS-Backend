...
exports.createReview = async (req, res) => {
  try {
    ...
    // Notify MC about the new review
    ...
    await notificationService.create(
      {
        ...
        body: `You received a ${rating}-star review for "${booking.eventName}"`,
        type: "Social",
        relatedId: review._id,
        relatedModel: "Review",
        linkAction: `/m/reviews`,
      },
      io,
    );
...
exports.getMCReviews = async (req, res) => {
  try {
    const reviews = await Review.find({ mc: req.params.mcId }).populate(
      "client",
      "name avatar",
    );
    res.status(200).json({
