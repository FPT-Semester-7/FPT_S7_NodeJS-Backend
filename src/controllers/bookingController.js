...
exports.createBooking = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ status: "success", data: { booking: new BookingDTO(booking) } });
  } catch (err) {
...
exports.getClientBookings = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      data: { bookings: bookings.map((booking) => new BookingDTO(booking)) },
    });
  } catch (err) {
    res.status(400).json({ status: "fail", message: err.message });
  }
...
exports.getMCBookings = async (req, res) => {
  try {
    const bookings = await bookingService.getMCBookings(req.user.id);
    res.status(200).json({
      status: "success",
      results: bookings.length,
      data: { bookings: bookings.map((booking) => new BookingDTO(booking)) },
